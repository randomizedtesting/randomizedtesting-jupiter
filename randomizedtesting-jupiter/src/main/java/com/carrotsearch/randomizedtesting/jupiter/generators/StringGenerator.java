...
public abstract class StringGenerator {
  ...
  /**
   * @return Returns a string of variable length between <code>minCodeUnits</code> (inclusive) and
   *     <code>maxCodeUnits</code> (inclusive) length. Code units are essentially an equivalent of
   *     <code>char</code> type, see {@link String} class for explanation.
   * @param minCodeUnits Minimum number of code units (inclusive).
   * @param maxCodeUnits Maximum number of code units (inclusive).
   * @throws IllegalArgumentException Thrown if the generator cannot emit random string of the given
   *     unit length. For example a generator emitting only extended unicodeGenerator plane
   *     characters (encoded as surrogate pairs) will not be able to emit an odd number of code
   *     units.
   */
  public abstract String ofCodeUnitsLength(Random r, int minCodeUnits, int maxCodeUnits);

  /**
   * @return Returns a string of variable length between <code>minCodePoints</code> (inclusive) and
   *     <code>maxCodePoints</code> (inclusive) length. Code points are full unicodeGenerator
