...
/**
 * Utility classes for selecting random numbers from within a range or the numeric domain for a
 * given type.
 *
 * @see BiasedNumbers
 */
public final class RandomNumbers {
  ...
  public static long randomLongInRange(Random r, long min, long max) {
    ...
    if (range < 0) {
      ...
      if (range == Long.MIN_VALUE) {
        ...
        long second =
            range == Long.MAX_VALUE ? (r.nextLong() & Long.MAX_VALUE) : nextLong(r, range + 1);
  ...
  /**
   * Similar to {@link Random#nextInt(int)}, but returns a long between 0 (inclusive) and <code>n
   * </code> (exclusive).
   *
