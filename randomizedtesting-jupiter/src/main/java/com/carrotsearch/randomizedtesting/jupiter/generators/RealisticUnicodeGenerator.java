...
public class RealisticUnicodeGenerator extends StringGenerator {
  ...
  public String ofCodeUnitsLength(Random r, int minCodeUnits, int maxCodeUnits) {
    ...
    while (length > 0) {
      int cp = RandomNumbers.randomIntInRange(r, blockStarts[block], blockEnds[block]);
  ...
  public String ofCodePointsLength(Random r, int minCodePoints, int maxCodePoints) {
    final int length = RandomNumbers.randomIntInRange(r, minCodePoints, maxCodePoints);
    final int block = r.nextInt(blockStarts.length);
    final StringBuilder sb = new StringBuilder();
    for (int i = 0; i < length; i++)
      sb.appendCodePoint(RandomNumbers.randomIntInRange(r, blockStarts[block], blockEnds[block]));
    return sb.toString();
  }
}
